...
object ZConfig {
  ...
  def getLocalDocRoot(): String = getString("local.doc.root", "")

  def getDocRoot(): String = getString("doc.root", "")
  ...
  def getStringMap(s: String) = {
    config.getObject(s).unwrapped().map(e => (e._1, if(e._2 !=null) e._2.toString else null)).toMap
  }

  def getMapList(s: String) = config.getObjectList(s).toSeq.map(_.unwrapped().toMap)
